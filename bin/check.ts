...
const itemsWithoutLocation = items.filter((item) => !item.location && item.rarity !== 'Common').map((item) => item.url);
...
const unusedLocations = allLocations.filter((location) => !usedLocations.has(location));
